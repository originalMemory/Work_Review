...
test('前端应向日报生成与工作助手透传当前 locale，并让日期输入跟随语言切换', async () => {
  ...
  assert.match(reportSource, /invoke\('generate_report', \{ date: selectedDate, force, locale: currentLocale, deviceId: \$selectedDeviceId \}\)/);
  assert.match(reportSource, /invoke\('get_saved_report', \{ date: selectedDate, locale: currentLocale, deviceId: \$selectedDeviceId \}\)/);
  assert.match(
    reportSource,
    /if \(!savedReport && previousReport\?\.date === selectedDate && previousReport\?\.content\)[\s\S]*invoke\('generate_report', \{ date: selectedDate, force: false, locale: currentLocale, deviceId: \$selectedDeviceId \}\)/,
  );
